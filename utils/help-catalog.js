...
function group(category, query) {
  ...
  const sections = visibleSections()
  return BROWSABLE.filter((item) => !HIDE_VIP || item.id !== 'vip').map((sectionCategory) => {
    const matched = sections.filter((item) => item.category === sectionCategory.id).map(toListItem)
    return matched.length ? { id: sectionCategory.id, label: sectionCategory.label, sections: matched } : null
  }).filter(Boolean)
}
...
function relatedSections(sectionId) {
  ...
  return visibleSections().filter((item) => item.id !== current.id && item.category === current.category)
    .slice(0, 3)
    .map(toListItem)
}
